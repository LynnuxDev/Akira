...
const functions: CustomFunction[] = [
  {
    ...
    code: `
      ...
      $description[$replace[$replace[$replace[$replace[$replace[$replace[$env[result;$get[error];description];{{prefix}};$if[$getUserVar[prefix;$get[author];false]!=false;$getUserVar[prefix;$get[author];$getGlobalVar[prefix]];$getUserVar[prefix;$guildID;$getGlobalVar[prefix]]];-1];{example};$commandInfo[messageCreate;$get[origin];usage];-1];{messageOne};$message[1];-1];{{client}};$clientID;-1];{{command}};$commandInfo[messageCreate;$get[origin];name];-1];{{messageZero}};$message[0];-1]]
    `
  }
];
