...
const commands: Command[] = [
  {
    ...
    code: `
      ...
      $switch[$get[option];
        $case[not-given;$customError[717;perms]]
        $case[show;
          $if[$message[1]==;
            $customError[802;perms]$let[type;error]
          ;
            $if[$findGuildChannel[$guildID;$message[1]]!=;
              $let[type;channel]$let[query;$findGuildChannel[$guildID;$message[1]]]
            ;
              $if[$findRole[$guildID;$message[1]]!=;
                $if[$findRole[$guildID;$message[1]]==$guildID;$customError[718;perms]$let[type;error];$let[type;role]$let[query;$findRole[$guildID;$message[1]]]]
              ;
                $if[$findMember[$guildID;$message[1];false]!=;
                  $let[type;user]$let[query;$findMember[$guildID;$message[1];false]]
                ;$customError[802;perms]$let[type;error]]
              ]
            ]
            $if[$get[type]!=error;
              $switch[$get[type];
                $case[user;
                  $title[@$userGlobalName[$get[query]]'s Permission list:] $getColor
                  $thumbnail[$userAvatar[$get[query]]]
                  $footer[These perms do not include Channel/Role perms overrides.]
                ]
                $case[role;
                  $title[@$roleName[$guildID;$get[query]] Permission list:] $getColor
                ...
                $case[channel;$title[#$channelName[$get[query]] Permission list:] $footer[These perms do not include User/Role perms overrides.]]
                $case[default;$customError[708;perms]]
              ]
          ...
          $if[$message[1]!=;
            $if[$containsCommand[$message[1]]==true;
              $if[$findGuildChannel[$guildID;$message[2]]!=;
                $let[type;channel]$let[query;$findGuildChannel[$guildID;$message[2]]]
              ;
                $if[$findRole[$guildID;$message[2]]!=;
                  $if[$findRole[$guildID;$message[2]]==$guildID;$customError[718;perms]$let[type;error];$let[type;role]$let[query;$findRole[$guildID;$message[2]]]]
                ;
                  $if[$findMember[$guildID;$message[2];false]!=;$let[type;user]$let[query;$findMember[$guildID;$message[2];false]];$customError[802;perms]$let[type;error]]
                ]
              ]
              $if[$get[type]!=error;
                $if[$checkContains[$getGuildVar[perms~$toLowercase[$message[1]];$customEncrypt[encrypt;$guildID]];+ $callFunction[customEncrypt;encrypt;$get[query]]]!=true;
                  ...
                  $title[Permissions Changed:] $getColor
            ...
            $if[$containsCommand[$message[1]]==true;
              ...
              $if[$get[type]!=error;
                $if[$checkContains[$getGuildVar[perms~$get[query];$guildID];- $toLowercase[$message[1]]]==true;
                  $let[error;min]
                ;
                  $if[$checkContains[$getGuildVar[perms~$get[query];$guildID];+ $toLowercase[$message[1]]]==true;$let[error;plus];$customError[804;perms]$let[error;true]]
                ]
                $if[$get[error]!=true;
                  $if[$get[error]==min;$setGuildVar[perms~$get[query];$replace[$replace[$getGuildVar[perm~$get[query];$guildID];- $toLowercase[$message[1]],;;-1];- $toLowercase[$message[1]];;-1];$guildID];$setGuildVar[perms~$get[query];$replace[$replace[$getGuildVar[perm~$get[query];$guildID];+ $toLowercase[$message[1]],;;-1];+ $toLowercase[$message[1]];;-1];$guildID]]
              ...
              $if[$containsModule[$message[1]]==true;
                ...
                ;
                  $if[$findRole[$guildID;$message[2]]!=;
                    $if[$findRole[$guildID;$message[2]]==$guildID;$customError[718;perms]$let[type;error];$let[type;role]$let[query;$findRole[$guildID;$message[2]]]]
                  ;
                    $if[$findMember[$guildID;$message[2];false]!=;$let[type;user]$let[query;$findMember[$guildID;$message[2];false]];$customError[805;perms]$let[type;error]]
                  ]
                ]
                $if[$get[type]!=error;
                  $if[$checkContains[$getGuildVar[perms~$get[query];$guildID];- $toLowercase[$message[1]]]==true;$let[error;min];$if[$checkContains[$getGuildVar[perms~$get[query];$guildID];+ $toLowercase[$message[1]]]==true;$let[error;plus];$customError[804;perms]$let[error;true]]]
                  $if[$get[error]!=true;
                    $if[$get[error]==min;$setGuildVar[perms~$get[query];$replace[$replace[$getGuildVar[perm~$get[query];$guildID];- $toLowercase[$message[1]],;;-1];- $toLowercase[$message[1]];;-1];$guildID];$setGuildVar[perms~$get[query];$replace[$replace[$getGuildVar[perm~$get[query];$guildID];+ $toLowercase[$message[1]],;;-1];+ $toLowercase[$message[1]];;-1];$guildID]]
                    $color[$if[$getUserVar[color;$get[author];false]!=false;$getUserVar[color;$get[author];#ff47ff];$getUserVar[color;$guildID;#ff47ff]]]
                    $title[Permissions unset:]
          ...
          $if[$message[1]!=;
            ...
            ;
              $if[$findRole[$guildID;$message[1]]!=;
                $if[$findRole[$guildID;$message[1]]==$guildID;$customError[718;perms]$let[type;error];$let[type;role]$let[query;$findRole[$guildID;$message[1]]]]
              ;
                $if[$findMember[$guildID;$message[1];false]!=;$let[type;user]$let[query;$findMember[$guildID;$message[1];false]];$customError[805;perms]$let[type;error]]
              ]
            ]
            $if[$get[type]!=error;
              $title[Permissions cleared:] $getColor
        ...
        $case[reply;
          $switch[$toLowerCase[$message[1]];
            $case[react;
              $title[Reply setting changed!] $getColor
            ...
            $case[default;$customError[801;perms]]
          ]
        ]
        $case[default;$customError[718;perms]]
      ]
    `
  }
];
